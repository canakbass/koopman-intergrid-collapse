...
def multirate_reinit(model, batch_fn, g, n_seq=256, K=8, tau=None):
    """batch_fn(n, K, g, mode) ; mode ∈ {regular, d2}. tau verilirse: yalnızca Δt2 artığı < tau olan modlar kilitlenir
    (diğerleri ana dal değeriyle serbest); hiçbiri geçmezse model DEĞİŞTİRİLMEZ."""
    with torch.no_grad():
    ...
    lifted, mi = multirate_lift(om, np.einsum("ij,ntj->nti", np.linalg.inv(P), ze2))
    info.update(mi); info["dmd_principal"] = [float(w) for w in om]
    if tau is None:
        info["dmd_omegas"] = [float(w) for w in lifted]
        with torch.no_grad(): _install(model, P, lifted)
        return info
    ok = [bool(r == r and r < tau and abs(w) > 1e-9) for r, w in zip(mi["mr_residual"], lifted)]
    info["gate_pass"] = ok
    if not any(ok):
        info["gated_skip"] = True; return info
    final = np.where(ok, lifted, om); info["dmd_omegas"] = [float(w) for w in final]; info["gated_skip"] = False
    with torch.no_grad(): _install(model, P, final)
    mask = torch.tensor([0.0 if o else 1.0 for o in ok], device=model.omega.device)
...
def partial_install_info(model, batch_fn, g, band=(1.5, 4.0), n_seq=256, K=8, tau=None):
    with torch.no_grad():
        x1, _ = batch_fn(n_seq, K, g, "regular"); x2, _ = batch_fn(n_seq, K, g, "d2")
        enc = lambda x: model.enc(x.reshape(-1, 1, data.H, data.H)).reshape(n_seq, K + 1, -1).double().cpu().numpy()
        ze1, ze2 = enc(x1), enc(x2)
        ...
        if tau is not None and not r < tau:          # faz tutarsız: kilitleme iptal, model değişmez
            info.update(dict(gated_skip=True, partial_pair=int(j), partial_omega=w, partial_residual=r, partial_candidates=table))
            return info
        om = om.copy(); om[j] = w
        _install(model, P, om)
    mask = torch.ones_like(model.omega); mask[j] = 0.0                # yalnızca seçilen çift dondurulur
    model.omega.register_hook(lambda gr: gr * mask)
    info.update(dict(gated_skip=False, partial_pair=int(j), partial_omega=w, partial_residual=r, partial_candidates=table, dmd_omegas=[float(x) for x in om]))
    return info
