...
def dmd_modal(ze, n_osc, n_static, dt=1.0, harmonic=False):
    """ze (N,T,d) numpy, düzenli ızgara. Dönüş: P (d×d, sütunlar = yeni koordinat tabanı), ω (n_osc,), bilgi"""
    N, T, d = ze.shape
    X = ze[:, :-1].reshape(-1, d).T; Y = ze[:, 1:].reshape(-1, d).T
    A = Y @ np.linalg.pinv(X, rcond=1e-8)
    lam, V = np.linalg.eig(A)
    amp = np.abs(np.linalg.pinv(V) @ X).mean(1) * np.linalg.norm(V, axis=0)
    cplx = [i for i in np.argsort(-amp) if lam[i].imag > 1e-6]            # her eşlenik çiftten Im>0 olan
    real = [i for i in range(d) if abs(lam[i].imag) <= 1e-6]
    chosen, rest_pairs = cplx[:n_osc], cplx[n_osc:]
    cols, om = [], []
    for i in chosen:
        v = V[:, i] / np.linalg.norm(V[:, i]); cols += [v.real, v.imag]; om.append(-np.angle(lam[i]) / dt)   # ana dal
    pool = [c for i in rest_pairs for c in (V[:, i].real, V[:, i].imag)] + [V[:, i].real for i in real]
    pool = [c / np.linalg.norm(c) for c in pool]
    while len(om) < n_osc: cols += [pool.pop(0), pool.pop(0)]; om.append(0.0)
    cols += pool[:n_static]
    P = np.stack(cols, 1)
    lift = {}
    if harmonic:
...
if __name__ == "__main__":
    import sys; sys.exit(0 if selftest() and selftest_lift() and selftest_multirate() else 1)
